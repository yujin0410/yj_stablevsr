    ...
    def __call__(
        ...
        with self.progress_bar(total=len(timesteps)*len(images)) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                dtcwt_cond = None  # [수정] 매 timestep마다 안전하게 초기화
                ...
                for num_image, data in enumerate(images):
                    ...
                    if num_image != 0: # if it's not the first image
                        ...
                        f_flow_cur = flows[num_image - 1]
                        warped_prev_est = of.flow_warp(x0_est, f_flow_cur, interp_mode=interp_mode) # warp it to current frame
                        
                        if sft_module is not None and dtcwt_xfm is not None:
                            with torch.no_grad():
                                _, Yh_prev_list = dtcwt_xfm(x0_est.float())
                                f_flow_chw = f_flow_cur.permute(0, 3, 1, 2)  # (B,H,W,2) → (B,2,H,W)
                                Yh_warped = warp_dtcwt_high_bands(Yh_prev_list, f_flow_chw.float())
                                ...
                                dtcwt_cond = dtcwt_cond.to(dtype=self.unet.dtype)
                    # ==========================================================
                        ...
                        if sft_module is not None and dtcwt_cond is not None:
                            ...
                            for sample in down_block_res_samples:
                                if sample.shape[1] == 320:
                                    ...
                                    mask = torch.tensor(float(use_dtcwt_cond), device=sample.device, dtype=sample.dtype)
                                    final_sample = refined_sample * mask + sample * (1.0 - mask)
                                    modified_down_block_res_samples.append(final_sample.to(dtype=self.unet.dtype))
                                else:
                                    modified_down_block_res_samples.append(sample.to(dtype=self.unet.dtype))
                        
                            down_block_res_samples = modified_down_block_res_samples 


                        if guess_mode and do_classifier_free_guidance:
                            # Infered ControlNet only for the conditional batch.
                            down_block_res_samples = [torch.cat([torch.zeros_like(d), d]) for d in down_block_res_samples]
                            mid_block_res_sample = torch.cat([to
        ...
        if has_nsfw_concept is None:
            do_denormalize = [True] * images[0].shape[0]  
        else:
            do_denormalize = [not has_nsfw for has_nsfw in has_nsfw_concept]

        images = [self.image_processor.postprocess(img, output_type=output_type, do_denormalize=do_denormalize) for img in images]
