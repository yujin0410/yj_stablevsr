...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        sft_module_unwrapped = accelerator.unwrap_model(sft_module)
        torch.save(sft_module_unwrapped.state_dict(), os.path.join(args.output_dir, "sft_module.pth"))
        logger.info(f"SFT weights successfully saved to {args.output_dir}")
        if args.push_to_hub:
            save_model_card(
                repo_id,
